 ...
 * @version: 
 * @Author: sueRimn
 * @Date: 2019-10-09 11:22:36
 * @LastEditors: sueRimn
 * @LastEditTime: 2019-10-15 11:45:06
 */
...
#define MQTT_HOST "192.168.0.34"		//ip地址
#define MQTT_PORT 1883					//端口号
#define MQTT_USER "admin"				//用户名
#define MQTT_PASS "public"			//密码
#define MQTT_CLIENT_ID "jiangfeng"		//客户端标识
...
void iot_mqtt_init(Cloud_MQTT_t *piot_mqtt) 
{
    ...
    sprintf(piot_mqtt->sub_topic, "%s%s/sub", gateway.model, gateway.company);	//要订阅的主题s
    printf("subscribe:%s\n", piot_mqtt->sub_topic);

    sprintf(piot_mqtt->pub_topic, "%s%s/pub", gateway.model, gateway.company);	//要发布的主题
    printf("pub:%s\n", piot_mqtt->pub_topic);
